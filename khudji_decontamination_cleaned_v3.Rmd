...
bleach <- read_delim("/path_to_wd/proteinGroups.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+"))  %>%
  filter(!str_detect(protein_names, "Trypsin")) %>%
  filter(!str_detect(protein_i_ds, "CON__P00761")) %>%
...
```{r}
# Load dataset
original <- read_delim("/path_to_wd/evidence.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
  filter(!str_detect(protein_names, "Trypsin")) %>%
...
bleach <- read_delim("/path_to_wd/evidence.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+"))  %>%
  filter(!str_detect(protein_names, "Trypsin")) %>%
  filter(!str_detect(proteins, "CON__P00761")) %>%
...
```{r}
# Load data
deamidation <- read.csv("/path_to_wd/deamidation_khudji_manuscript.txt", sep="") %>%
  clean_names()
...
This part is based on output files from PEAKS11, which can be found on the ProteomeXchange Consortium via the PRIDE partner repository with the dataset identifier PXD050370. To run the code, you need to download the files from PRIDE, and change the paths in the code blocks to point to the data files. The reconstruction is done twice, once for each extraction method, which is then checked and a consensus sequence created manually (see publication for details). 

```{r echo=FALSE, message=FALSE, warning=FALSE}
# Load protein-level data
spider_proteins <- read_delim("/path_to_wd/spider.proteins.csv", 
...
peptide <- read_delim("/path_to_wd/spider.protein-peptides.csv", delim = ",", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() 
...
writeFasta(new_fasta, "/path_to_wd/output.fasta")
```
