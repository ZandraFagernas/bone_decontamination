...
The metadata file is in supplementary information of the publication, called supplementary_data_1.txt. All analyses are based on MaxQuant output, and can be found on the ProteomeXchange Consortium via the PRIDE partner repository with the dataset identifier PXD050393. To run the code, you need to download the files from PRIDE, and change the paths in the code blocks to point to the data files. Note that  the figures may have been manually edited for improved visuals after exporting. Not all figures that are coded for here are included in the manuscript (comments in the code will show which ones are), as I like to visualize data just to understand it.
...
meta <- read_delim("/path_to_wd/supplemetary_data_1.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() 
```
...
```{r}
# Load dataset
proteins <- read_delim("/path_to_wd/proteinGroups.txt", 
...
```{r}
# Load dataset
peptide <- read_delim("/path_to_wd/evidence.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
...
### You need to make this one yourself, but it's just going through the contaminants manually and noting down what they are
cont <- read_delim("/path_to_wd/identified_contaminants.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
...
# Load dataset
human <- read_delim("/path_to_wd/proteinGroups.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
...
```{r}
# Peptide count
lt <- read_delim("/path_to_wd/proteinGroups.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Load peptide files
peptide <- read_delim("/path_to_wd/peptides.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
...
evidence <- read_delim("/path_to_wd/evidence.txt", delim = "\t") %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
  filter(intensity > 0) %>%
...
```{r}
# Load evidence file and clean, filter for horse-only peptides
evidence_uv <- read_delim("/path_to_wd/evidence.txt", delim = "\t") %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
  filter(intensity > 0) %>%
  filter(str_detect(proteins, "HORSE")) %>%
...
deam <- read.csv("/path_to_wd/deamidation_decont.txt", sep="") %>%
  clean_names()
...
```{r}
# Load dataset
intensity <- read_delim("/path_to_wd/evidence.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
  filter(!str_detect(proteins, "CANLF")) %>%
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
gene <- read_delim("/path_to_wd/proteinGroups.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
  #filter(str_detect(protein_i_ds, "CANLF|HORSE")) %>%
...
horse <- read_delim("/path_to_wd/proteinGroups.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names()  %>% 
  filter(!reverse %in% c("+"))  %>% 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Load datasets and clean
gene_horse <- read_delim("/path_to_wd/proteinGroups.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names()  %>% 
  filter(!reverse %in% c("+"))  %>% 
...
```{r}
# Load summary file
summary <- read_delim("/path_to_wd/summary.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  janitor::clean_names() %>%
