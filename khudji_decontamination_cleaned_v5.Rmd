...
length_plot <- ggplot(length_summary, aes(x=length, y=total_intensity, fill=experiment)) + 
...
Here, you first need to calculate deamidation from the evidence files using the script published in:
...
The evidence.txt files from both MaxQuant runs were manually combined and used as input for the deamidation script. When you have run the scrip above, move the output into the same folder as where you keep this Markdown file.

```{r}
# Load data
deamidation <- read.csv("Deamidation.txt", sep="") %>%
  clean_names()
...
spider_proteins <- read_delim("bleach_PEAKS/bleach_fdr1/spider.proteins.csv", 
...
peptide <- read_delim("bleach_PEAKS/bleach_fdr1/spider.protein-peptides.csv", delim = ",", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Load protein-level data
spider_proteins <- read_delim("original_PEAKS/original_fdr1/spider.proteins.csv", 
    delim = ",", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Load peptide-level data
peptide <- read_delim("original_PEAKS/original_fdr1/spider.protein-peptides.csv", delim = ",", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() 
