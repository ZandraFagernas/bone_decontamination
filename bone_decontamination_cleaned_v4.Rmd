...
```{r}
# Load dataset
proteins <- read_delim("equus_maxquant/proteinGroups.txt", 
...
```{r}
# Load dataset
peptide <- read_delim("equus_maxquant/evidence.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
...
```{r}
# Peptide count
lt <- read_delim("equus_maxquant/proteinGroups.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Load peptide files
peptide <- read_delim("equus_maxquant/peptides.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
...
evidence <- read_delim("equus_maxquant/evidence.txt", delim = "\t") %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
  filter(intensity > 0) %>%
...
```{r}
# Load dataset
intensity <- read_delim("equus_maxquant/evidence.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
  filter(!str_detect(proteins, "CANLF")) %>%
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
gene <- read_delim("equus_maxquant/proteinGroups.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
  #filter(str_detect(protein_i_ds, "CANLF|HORSE")) %>%
