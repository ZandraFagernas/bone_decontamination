...
```{r}
# Load dataset
peptide <- read_delim("<PATH_TO>/evidence.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
...
### This is just a list where I went through the identified contaminants and assigned what they are
cont <- read_delim("<PATH_TO>/identified_contaminants.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
...
# Load dataset
human <- read_delim("<PATH_TO>/proteinGroups.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
...
Note that UV-induced modifications are from a different MaxQuant run!
...
deam <- read.csv("<PATH_TO>/deamidation_decont.txt", sep="") %>%
  clean_names()
...
```{r}
# Load dataset
intensity <- read_delim("<PATH_TO>/evidence.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names() %>%
  filter(!reverse %in% c("+")) %>%
  filter(!str_detect(proteins, "CANLF")) %>%
...
From MaxQuant run without dog proteome.
...
# Same for PSMs
psms <- read_delim("<PATH_TO>/evidence.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  clean_names()  %>% 
  filter(!reverse %in% c("+")) %>%
...
Horse-only MaxQuant run.

```{r}
# Load summary file
summary <- read_delim("<PATH_TO>/summary.txt", 
    delim = "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  janitor::clean_names() %>%
...
# Figure for the number of MS2 spectra
...
# Figure for the number of identified MS2 spectra
...
# Figure for the percentage of identified MS2 spectra
...
# Figure for the identified peptides
